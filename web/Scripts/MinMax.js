...
function minMax(depth, Board, turn) {
    if (depth==0) {
	...
	for (var i=0; i<7; i++) {
		...
		TempBoard[move][i]=tmpPiece
		alpha=Math.max(alpha, -minMax(depth-1, TempBoard, turn+1))
	    }
	}
	return alpha
    }
...
function compMoveMinMax(depth, piece) {
    var compMove
    var value=-100000
    if (piece==" O ") {
	value=100000
	...
	var move=MakeMove(Board, i)
		...
		TempBoard[move][i]=tmpPiece
		alpha=Math.max(alpha, -minMax(depth-1, TempBoard, turn+1))
		if (alpha>value&&piece==" X ") {
		    
		}
		else if (alpha<value&&piece==" O ") {
